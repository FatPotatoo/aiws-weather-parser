...
base_dir = Path(r"C:\Users\suyas\Downloads\AIWS\2026")
extractor_dir = Path(r"c:\xampp\htdocs\aiws-weather-parser\fireworks-weather-extractor")

quarter_files = {
    1: extractor_dir / "output_q1_2026.csv",
    2: extractor_dir / "output_q2_2026.csv",
    3: extractor_dir / "output_q3_2026.csv",
    4: extractor_dir / "output_q4_2026.csv",
}

resume_file = extractor_dir / ".run_2026_quarters.resume"

def get_file_quarter(path: Path) -> int | None:
    # Match 8 digits representing the date, e.g. "AIWS 20260314.docx"
...
def main():
    ...
    print("Scanning 2026 folder recursively for .docx files...")
    all_files = []
    ...
    print(f"Found {len(all_files)} bulletins in 2026 to process.")
    ...
    try:
        for idx, path in enumerate(pending_files, 1):
            ...
            print(f"[{idx}/{len(pending_files)}] 2026 - Q{q} - Extracting from: {path.name}...")
