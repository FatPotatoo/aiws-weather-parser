...
3. **Run Extraction**:
   Run the extractor on the bulletin folder. By default, it will check the local folder or the sibling repository path (`../aiws-weather-parser/AIWS2025`):
   ```bash
   python runner.py --out output_fireworks.csv --limit 2
   ```

   Arguments:
   - `--folder` / `-d`: Path to the folder containing `.docx` bulletins.
   - `--out` / `-o`: Output CSV file path.
   - `--limit`: Process at most N files (for dry/test runs).
   - `--model`: Specific model identifier (defaults to `accounts/fireworks/models/glm-5p2`).
   - `--delay`: Seconds of delay between requests (default is 1.0s).
